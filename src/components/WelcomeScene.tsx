import React from 'react';
import Image from 'next/image';

interface WelcomeSceneProps {
  onNext: () => void;
}

const WelcomeScene: React.FC<WelcomeSceneProps> = ({ onNext }) => {
  return (
    <div className="h-screen w-full flex flex-col items-center justify-between p-4 bg-white relative overflow-hidden">
      {/* Yellow background shape */}
      <div 
        className="absolute inset-0 bg-[#FFDE3D] -z-10"
        style={{
          clipPath: 'polygon(0 0, 100% 0, 100% 45%, 0 45%)',
          height: '100%',
          width: '100%',
        }}
      />
      
      <div className="w-full max-w-md mx-auto text-center flex flex-col items-center justify-center flex-grow px-4">
        {/* Logo */}
        <div className="mb-4 sm:mb-6 w-16 h-16 sm:w-20 sm:h-20 relative">
          <Image 
            src="/logorevou.png" 
            alt="RevoU Logo" 
            fill
            sizes="(max-width: 640px) 64px, 80px"
            className="object-contain"
            priority
          />
        </div>
        
        {/* Headline */}
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-4 sm:mb-6 px-2">
          Selamat Datang, Calon Digital Marketer!
        </h1>
        
        {/* Animated GIF */}
        <div className="w-full max-w-[280px] sm:max-w-[320px] mx-auto my-4 sm:my-6 aspect-square relative">
          <Image
            src="/GIF/ezgif.com-animated-gif-maker.gif"
            alt="Digital Marketing"
            fill
            sizes="(max-width: 640px) 280px, 320px"
            className="object-contain"
            priority
          />
        </div>
        
        {/* Subheadline */}
        <p className="text-gray-600 text-base sm:text-lg mb-6 sm:mb-8 px-2">
          Cari tahu tipe Digital Marketer seperti apakah kamu?
        </p>
      </div>
      
      {/* CTA Button Container - Fixed at bottom */}
      <div className="w-full max-w-md mx-auto px-4 pb-6 sm:pb-8 pt-4 bg-white">
        <button 
          className="bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg transition duration-200 w-full max-w-xs mx-auto block"
          onClick={onNext}
        >
          Mulai Sekarang
        </button>
      </div>
    </div>
  );
};

export default WelcomeScene;
